...
function fromRow(b:any): Booking {
  return {
    id:b.id, manageToken:b.manage_token,
    ...
    customerPhone:b.customer_phone || undefined, note:b.note || undefined,
    surveyData:b.survey_data || {},
    source:b.source || undefined, campaign:b.campaign || undefined,
    start:b.start_at, end:b.end_at, timezone:b.timezone,
    status:b.status, crmStage:b.crm_status,
    meetUrl:b.meeting_url, calendarEventId:b.calendar_event_id,
...
export async function getPublicBookedSlots(start:string,end:string) {
  const supabase = await createClient();
  const { data, error } = await supabase.from('bookings').select('start_at,end_at,status')
    .eq('status','confirmed').lt('start_at',end).gt('end_at',start);
  if (error) throw error;
  return data || [];
}
export async function hasConflict(start:string,end:string) {
  return (await getPublicBookedSlots(start,end)).length > 0;
}
export async function getAvailability(hostSlug:string, weekday:number) {
  const supabase = await createClient();
  const { data, error } = await supabase.from('availability').select('start_time,end_time,timezone')
    .eq('host_slug',hostSlug).eq('weekday',weekday).eq('is_active',true).order('start_time');
  if (error) throw error;
  return data || [];
}
export async function getBusyBlocks(hostSlug:string,start:string,end:string) {
  const supabase = await createClient();
  const { data, error } = await supabase.from('busy_blocks').select('start_at,end_at')
    .eq('host_slug',hostSlug).lt('start_at',end).gt('end_at',start);
  if (error) throw error;
  return data || [];
}
export async function saveBooking(booking: Booking) {
  ...
  const { error } = await supabase.from('bookings').insert({
    id:booking.id, customer_name:booking.customerName, customer_email:booking.customerEmail,
    customer_phone:booking.customerPhone || null, note:booking.note || null,
    survey_data:booking.surveyData || {}, start_at:booking.start, end_at:booking.end,
    timezone:booking.timezone, status:booking.status, crm_status:booking.crmStage,
    source:booking.source || null, campaign:booking.campaign || null,
    meeting_url:booking.meetUrl || null, calendar_event_id:booking.calendarEventId || null,
