...
const schema = z.object({
  ...
  customerPhone: z.string().optional(), note: z.string().optional(), source: z.string().optional(), campaign: z.string().optional(),
...
export async function POST(request: Request) {
  try {
    ...
    const calendar = await createCalendarEvent({
      summary: `Tư vấn với ${body.customerName}`, description: body.note || "Booking từ webapp",
    ...
    const booking: Booking = {
      ...
      source: body.source, campaign: body.campaign, start: body.start, end: body.end, timezone: config.timezone,
      status: "confirmed", crmStage: "booked", meetUrl: calendar.meetUrl, calendarEventId: calendar.eventId,
      createdAt: now, updatedAt: now
    };
    await saveBooking(booking);
    await sendBookingEmails({ manageToken, hostName: config.hostName, hostEmail: config.hostEmail, customerName: body.customerName,
